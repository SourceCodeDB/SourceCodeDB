...
if(isset($_GET['id'])) {
	$id = $_GET['id'];
	$checkit = mysql_query("SELECT * FROM CodeInfo WHERE StringID = $id");
	$checkres = mysql_fetch_array($checkit);
	$useridcheck = $checkres['UserID'];
	$projtitle = $checkres['Title'];
	$projdesc = $checkres['Description'];
}
...
if (isset($_GET['id']) && isset($_SESSION['ID']) && $useridcheck != $_SESSION['ID']) {
	echo 'no hackz, sorry it seems like you got here by mistake?';
}
else
{
...
?>
<script>
  var editor = CodeMirror.fromTextArea($("#code1:last"), {
    lineNumbers: true,
    matchBrackets: true,
    mode: "htmlmixed"
  });
...
echo 'sorry you must be logged in to make a project';
